...
namespace App\Http\Controllers\Tenant\Api\Table\status;
...
class StatusController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $items = collect($paginator->items())->map(function (Status $status, int $index) use ($from) {
            return [
                'id' => $status->id,
                'sr_no' => $from + $index,
                'name' => $status->name,
                ...
                'can_delete' => checkPermission(['tenant.status.delete']),
                'can_edit' => checkPermission(['tenant.status.edit']),
    ...
    public function destroy(string $id): JsonResponse
    {
        ...
        $status = Status::where('tenant_id', $tenantId)->findOrFail($id);
        ...
        return response()->json([
            'message' => t('status_deleted_successfully'),
        ]);
    }
