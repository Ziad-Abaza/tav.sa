...
class CampaignExecutedController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = CampaignDetail::query()
            ...
            ->select([
                ...
                'campaign_details.response_message',
        ...
        if ($search = $request->get('q')) {
            $query->where(function ($q) use ($search) {
                $q->where('contact.firstname', 'like', "%{$search}%")
        ...
        $sort = in_array($sort, $this->sortable, true) ? $sort : 'id';
