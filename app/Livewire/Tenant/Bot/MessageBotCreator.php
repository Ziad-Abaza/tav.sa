...
class MessageBotCreator extends Component
{
        ...
        if (checkPermission(['tenant.message_bot.edit', 'tenant.message_bot.create'])) {
            ...
            try {
                ...
                if ($this->file) {
                    ...
                    $whatsappMediaId = $this->uploadMediaToWhatsApp($path);
                    $this->message_bot->whatsapp_media_id = $whatsappMediaId;
    ...
    public function removeFile()
    {
        ...
        if (! empty($this->message_bot->filename)) {
            ...
            $this->message_bot->whatsapp_media_id = null;
        }
     ...
     */
    private function uploadMediaToWhatsApp(string $storedPath): ?string
    {
        try {
            $filePath = storage_path('app/public/'.$storedPath);

            if (! file_exists($filePath)) {
                whatsapp_log('Message bot media file not found for WhatsApp upload', 'warning', [
            ...
            if ($mediaId) {
                whatsapp_log('Message bot media uploaded to WhatsApp successfully', 'info', [
                    'stored_path' => $storedPath,
                    'media_id' => $mediaId,
                    'tenant_id' => tenant_id(),
                ], null, tenant_id());
            } else {
                whatsapp_log('Message bot media upload to WhatsApp returned no ID', 'warning', [
                    'stored_path' => $storedPath,
            ...
            whatsapp_log('Message bot media upload to WhatsApp failed', 'error', [
                'stored_path' => $storedPath,
                'error' => $e->getMessage(),
                'tenant_id' => tenant_id(),
            ], $e, tenant_id());

            // Don't throw - allow save to succeed with fallback to URL
            return null;
        }
    }
