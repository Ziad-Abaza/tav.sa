...
class CsvCampaign extends Component
{
    ...
    public function save()
    {
        ...
        try {
            ...
            if ($this->file) {
                ...
                // Upload media to WhatsApp Cloud API immediately
                if (in_array($template['header_data_format'], ['IMAGE', 'VIDEO', 'DOCUMENT'])) {
                    $whatsappMediaId = $this->uploadMediaToWhatsApp(
                        $this->filelink,
                        $template['header_data_format']
                    );
                    // Store media_id for use in bulk campaign
                    $data['whatsapp_media_ids'] = $whatsappMediaId ? ['header' => $whatsappMediaId] : null;
                }
            }
            ...
            // Add whatsapp_media_ids to campaign data if available
            if (isset($data['whatsapp_media_ids'])) {
                $data['whatsapp_media_ids'] = $data['whatsapp_media_ids'];
            }
     ...
     *
     * @param  string  $filename  Relative path to file (e.g., 'tenant/123/csv/images/file.jpg')
    ...
    protected function uploadMediaToWhatsApp(string $filename, string $mediaType): ?string
    {
        try {
            ...
            if (! file_exists($filePath)) {
                whatsapp_log('Media file not found for WhatsApp upload (CSV Campaign)', 'error', [
                    'filename' => $filename,
                    'filepath' => $filePath,
                ], null, tenant_id());
            ...
            if (isset($data['id'])) {
                whatsapp_log('CSV Campaign media uploaded to WhatsApp Cloud API successfully', 'info', [
                    'filename' => $filename,
                    'media_id' => $data['id'],
            ...
            whatsapp_log('WhatsApp media upload failed - no ID returned (CSV Campaign)', 'error', [
                'filename' => $filename,
                'response' => $data,
            ], null, tenant_id());
            ...
            whatsapp_log('Error uploading media to WhatsApp Cloud API (CSV Campaign)', 'error', [
                'filename' => $filename,
                'error' => $e->getMessage(),
            ], $e, tenant_id());
