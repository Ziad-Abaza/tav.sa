...
class SchedulingServiceProvider extends ServiceProvider
{
    ...
    private function registerMessagingTasks(Schedule $schedule, string $timezone): void
    {
        ...
        // Process FB Messenger campaigns every 5 minutes
        $schedule->command('fb-campaigns:process-scheduled')
            ->everyMinute()
            ->timezone($timezone)
            ->withoutOverlapping(30)
            ->runInBackground()
            ->skip(function () {
                if (Cache::has('schedule:fb-campaigns:process-scheduled:running')) {
                    return true;
                }
            ...
            ->between('00:00', '23:59')
            ->before(function () {
                Cache::put('schedule:fb-campaigns:process-scheduled:running', true, 300);
            })
            ->after(function () {
                Cache::forget('schedule:fb-campaigns:process-scheduled:running');
            })
            ->onFailure(function () {
                Cache::forget('schedule:fb-campaigns:process-scheduled:running');
            });
