...
if (! function_exists('get_settings_classes')) {
    /**
     * Get all settings classes with in-memory caching.
     */
    function get_settings_classes(): array
    {
        ...
        $classes = Cache::remember('settings.classes', 3600, function () {
...
if (! function_exists('get_settings_by_group')) {
    ...
    function get_settings_by_group(string $group, mixed $default = null)
    {
        ...
        static $invalidationChecked = false;

        try {
            // Check for invalidation only once per request
            if (! $invalidationChecked) {
            ...
            // Return cached instance if available (in-memory cache)
...
if (! function_exists('get_setting')) {
     ...
     * Note: Settings are already cached by SettingsManager, no need for additional caching here.
     *
     * @throws \Corbital\Settings\Exceptions\SettingsException
     */
    function get_setting(string $key, mixed $default = null): mixed
    {
        try {
            ...
            // Direct property access - Settings are already cached by SettingsManager
