...
class TenantSettingsManager extends SettingsManager
{
     ...
     * Cached tenant hash.
     */
    protected ?string $cachedTenantHash = null;
    ...
    {
        // Cache tenant hash as it doesn't change per instance
        if ($this->cachedTenantHash === null) {
            $this->cachedTenantHash = hash('xxh3', (string) $this->tenantId);
        }

        $keyHash = hash('xxh3', $key);

        return "{$this->cachePrefix}tn_{$this->cachedTenantHash}_{$keyHash}";
    }
    ...
    /**
     * Get a setting with lock protection and optimized cache access.
     */
    protected function getWithLock(string $key, mixed $default = null): mixed
    {
        $cacheKey = $this->getCacheKey($key);

        // Single cache operation: get() returns null if not found
