...
class ModuleManager
{
    ...
    public function activate($name, array $activationStack = [], array $validationData = [])
    {
        ...
        if (! $this->isActive($name)) {
            ...
            // Publish module assets if build directory exists
            $this->publishModuleAssets($name);
    ...
    /**
     * Publish module assets by copying build files to public directory.
     *
     * @param  string  $name
     * @return void
     */
    public function publishModuleAssets($name)
    {
        try {
            $modulePath = base_path("Modules/{$name}");
            $sourcePath = "{$modulePath}/public/build";
            $destinationPath = public_path("modules/{$name}");

            // Only publish if module has a build directory
            ...
            app_log("Published assets for {$name} module", 'info', null, [
                'module' => $name,
                'source' => $sourcePath,
                'destination' => $destinationPath,
            ]);
        } catch (\Exception $e) {
            app_log("Failed to publish assets for {$name} module: ".$e->getMessage(), 'warning', $e, [
                'module' => $name,
            ]);
        }
    }
