...
class ModuleUpdateChecker
{
    ...
    private function extractUpdate(string $zipFile, string $type, string $mode, string $item_id): void
    {
        ...
        try {
            ...
            // Handle module-specific unused files cleanup after main extraction
            if ($type === 'main' && ! empty($item_id)) {
                $this->cleanupUnusedModuleFiles($item_id);
            }
    ...
    /**
     * Clean up unused module files after update extraction.
     *
     * Scans all module directories for unused-files.json.
     * If found, deletes each listed file, generates cleanup reports,
     * and removes the JSON file itself.
     */
    private function cleanupUnusedModuleFiles(string $item_id): void
    {
        ...
        foreach (File::directories($modulesDir) as $directory) {
            $unusedFilesPath = $directory.'/unused-files.json';

            if (! File::exists($unusedFilesPath)) {
                continue;
            }
            ...
            try {
                $filesList = json_decode(File::get($unusedFilesPath), true);

                if (json_last_error() !== JSON_ERROR_NONE || ! is_array($filesList)) {
                    continue;
                }
                ...
                $this->saveModuleCleanupReports($moduleName, $results);

                // Remove the unused-files.json itself after processing
                File::delete($unusedFilesPath);
            } catch (\Exception $e) {
                app_log('Module unused files cleanup failed', 'error', $e, [
                    'module' => $moduleName,
                ]);
            }
        }
    ...
    /**
     * Save cleanup results as JSON and Markdown report files.
     */
    private function saveModuleCleanupReports(string $moduleName, array $results): void
    {
        $slug = strtolower($moduleName);

        // Save JSON results
        File::put(
            storage_path("app/module-cleanup-results-{$slug}.json"),
            json_encode($results, JSON_PRETTY_PRINT)
        );

        // Build Markdown report
        $lines = [];
        $lines[] = "# {$moduleName} - File Cleanup Report";
        $lines[] = '';
        ...
        File::put(
            storage_path("app/module-cleanup-report-{$slug}.md"),
            implode("\r\n", $lines)
        );
    }
