...
onMounted(() => {
    window.addEventListener("custom-field-table-refresh", onTableRefresh);
...
function onDocumentMousedown(e) {
    ...
    if (!e.target.closest("[data-custom-field-dropdown]")) {
        closeDropdown();
    }
}
...
async function toggleActive(row) {
    ...
    try {
        const response = await axios.patch(
            `/${props.subdomain}/api/custom-fields/${row.id}/toggle-active`,
        );
...
// ─── required toggle (interactive PATCH) ─────────────────────────────────────

async function toggleRequired(row) {
    if (togglingRequired.has(row.id)) return;

    togglingRequired.add(row.id);

    try {
        const response = await axios.patch(
            `/${props.subdomain}/api/custom-fields/${row.id}/toggle-required`,
        );
        ...
        console.error("[CustomFieldDataTable] toggleRequired error:", e);
        const errorMessage = e.response?.data?.message || "Something went wrong";
        showNotification(errorMessage, "danger");
    } finally {
        togglingRequired.delete(row.id);
    }
}

// ─── show_on_table toggle (interactive PATCH) ─────────────────────────────────────

async function toggleShowOnTable(row) {
    if (togglingShowOnTable.has(row.id)) return;

    togglingShowOnTable.add(row.id);

    try {
        const response = await axios.patch(
            `/${props.subdomain}/api/custom-fields/${row.id}/toggle-show-on-table`,
        );
        ...
        console.error("[CustomFieldDataTable] toggleShowOnTable error:", e);
        const errorMessage = e.response?.data?.message || "Something went wrong";
        showNotification(errorMessage, "danger");
    } finally {
        togglingShowOnTable.delete(row.id);
    }
}
...
function confirmDelete(id) {
    dispatchLivewire("confirmDelete", { fieldId: id });
}
...
<template>
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <div class="p-4">
            <ResourceDataTable ref="table" resource="custom-fields" :subdomain="subdomain"
                storage-key="custom-fields_table">
                ...
                <template #is_required="{ row, value }">
                    <button
                        class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        :class="value ? 'bg-primary-500' : 'bg-gray-200 dark:bg-slate-600'" :disabled="togglingRequired.has(row.id)"
                        role="switch" :aria-checked="String(value)" @click="toggleRequired(row)">
                        <span
                            class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"
                            :class="value ? 'translate-x-4' : 'translate-x-0'" />
                    </button>
                ...
                <template #show_on_table="{ row, value }">
                    <button
                        class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        :class="value ? 'bg-primary-500' : 'bg-gray-200 dark:bg-slate-600'" :disabled="togglingShowOnTable.has(row.id)"
                        role="switch" :aria-checked="String(value)" @click="toggleShowOnTable(row)">
                        <span
                            class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"
                            :class="value ? 'translate-x-4' : 'translate-x-0'" />
                    </button>
    ...
    <Teleport to="body">
        <div v-if="activeDropdown !== null && selectedRow" data-custom-field-dropdown
            ...
            <!-- Edit -->
            <a v-if="selectedRow.can_edit" :href="`/${subdomain}/custom-fields/${activeDropdown}/edit`"
