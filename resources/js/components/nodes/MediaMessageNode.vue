...
const whatsappMediaId = ref(output.value.whatsapp_media_id || '');
const errors = ref({});
  ...
  props.data.output = [
    {
      ...
      whatsapp_media_id: whatsappMediaId.value,
    },
  ];
...
async function uploadFileToServer(file) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = errorText || `Upload failed with status: ${response.status}`;
    ...
    // Update with the public URL and WhatsApp media ID received from Laravel
    if (data && data.url) {
      mediaUrl.value = data.url; // This will be a public URL, not a blob
      // Store WhatsApp media ID to avoid 429 rate limiting when sending
      if (data.whatsapp_media_id) {
        whatsappMediaId.value = data.whatsapp_media_id;
      }
      updateNodeData();
    } else {
...
watch(mediaType, (newType) => {
  if (uploadedFile.value) {
    ...
    if (!mediaTypeData || !mediaTypeData.extensions.includes(fileExtension)) {
      ...
      fileName.value = '';
      mediaUrl.value = '';
      whatsappMediaId.value = ''; // Reset WhatsApp media ID when file is cleared
    }
  }
  updateNodeData();
});
