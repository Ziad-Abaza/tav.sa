...
function hasUnconnectedNodes(nodes, edges) {
    ...
    // Find unconnected nodes (excluding trigger nodes which are allowed to have no incoming connections)
    const unconnectedNodes = nodes.filter((node) => {
        // Trigger nodes should only have outgoing connections
        if (node.type === 'trigger') {
            return !nodesWithOutgoing.has(node.id);
        }
        // All other nodes must have at least one incoming connection
        return !nodesWithIncoming.has(node.id);
    });
    ...
    if (unconnectedNodes.length > 0) {
        const nodeLabels = unconnectedNodes
            ...
            .join(", ");
        showNotification(
            `The following nodes are not connected: ${nodeLabels}`,
            "danger"
        );
        return true;
    }
