<template>
    <div class="min-h-screen bg-gray-50 dark:bg-transparent">
        ...
        <CarouselEditor v-else-if="templateType === 'carousel'" :template="selectedTemplate" :categories="categories"
            :languages="languages" @close="closeEditor" @save="handleSave" @back="backToTemplate"
            :isSubmitting="loading" />

        <!-- Conditional Editors -->
        <TemplateEditor v-else :template="selectedTemplate" :categories="categories" :languages="languages"
            @close="closeEditor" @save="handleSave" @back="backToTemplate" :isSubmitting="loading" />
    </div>
</template>
...
const handleSave = async (templateData) => {
    try {
        ...
        let errorMessage = error.message || t("something_went_wrong");
