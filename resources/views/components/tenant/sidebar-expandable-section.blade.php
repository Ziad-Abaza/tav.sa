...
@if ($expandable && $title)
    <div x-data="{
        ...
        toggleSection() {
            ...
            localStorage.setItem('tenant_sidebar_section_{{ $sectionId }}', this.expanded ? '1' : '0');
        },
        init() {
            // Restore state from localStorage
            const stored = localStorage.getItem('tenant_sidebar_section_{{ $sectionId }}');
            if (stored !== null) {
        ...
        <button @click="toggleSection()" x-show="!isCollapsed"
        ...
        <div x-show="expanded" x-transition:enter="transition ease-out duration-200"
            ...
            <div
                :class="isCollapsed ? 'border-gray-200 dark:border-slate-600 space-y-1' :
                    'ml-2 border-l-2 border-gray-200 dark:border-slate-600 pl-4 space-y-1'">
                {{ $slot }}
            </div>
        </div>
...
@elseif ($title)
    ...
    <p x-show="!isCollapsed" x-transition:enter.duration.700ms
