<div>
    <x-slot:title>
        {{ !empty($contact['id']) ? t('edit_contact_title') : t('add_contact_title') }}
    </x-slot:title>

    <x-breadcrumb :items="[
        ['label' => t('dashboard'), 'route' => tenant_route('tenant.dashboard')],
        ['label' => t('contacts'), 'route' => tenant_route('tenant.contacts.list')],
        ['label' => !empty($contact['id']) ? t('edit_contact_title') : t('add_contact_title')]
    ]" />
        ...
        <form wire:submit.prevent="save">
            ...
            <x-card class="relative rounded-lg lg:w-3/4" x-cloak>
                <x-slot:content>
                    ...
                    @if (empty($contact['id']) && isset($this->hasReachedLimit) && $this->hasReachedLimit)
                    <div class="px-6 pt-4">
                    ...
                    <div class="flex justify-end space-x-3">
                        ...
                        <x-button.loading-button type="submit" target="save" wire:loading.attr="disabled">
                            <span wire:loading.remove wire:target="save">
                                {{ !empty($contact['id']) ? t('update_button') : t('add_button') }}
                            </span>
