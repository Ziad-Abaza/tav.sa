<div class="relative">
    ...
    <x-breadcrumb :items="[['label' => t('dashboard'), 'route' => route('admin.dashboard')], ['label' => t('languages')]]" />
    ...
    <div class="flex justify-start mb-3 items-center gap-2">
        ...
        <x-button.primary wire:click="syncLanguage" wire:loading.attr="disabled"
            ...
            <span wire:loading.remove wire:target="syncLanguage">{{ t('sync_languages') }}</span>
            <span wire:loading wire:target="syncLanguage" class="flex items-center">
    ...
    <div class="mt-8 lg:mt-0" >
         <div class="mt-8 lg:mt-0">
        <div wire:ignore>
            <div id="admin-languages-datatable"></div>
        </div>
    </div>
    </div>

    {{-- Add Language Modal --}}
    <x-modal.custom-modal :id="'showLanguageModal'" :maxWidth="'4xl'" wire:model="showLanguageModal">
        ...
        <form wire:submit.prevent="save" class="mt-4">
            <div class="px-6 space-y-3">
                ...
                <div class="border-t border-neutral-200 dark:border-neutral-500/30 pt-6">
                    ...
                    <p class="text-sm text-secondary-600 dark:text-secondary-400 mb-4">
                        Upload a JSON translation file. Files will be saved to resource/lang/translations directory.
                    </p>
                        ...
                        <div x-data="{ isDragging: false }" @dragover.prevent="isDragging = true"
                            ...
                            @if ($editFilePreview)
                                ...
                                <div class="space-y-3">
                                    <div class="flex items-center justify-center">
                                        <div
                                            class="w-12 h-12 bg-primary-100 dark:bg-primary-900/50 rounded-lg flex items-center justify-center">
                                    ...
                                    <div class="space-y-2">
                                        <p class="text-sm font-medium text-secondary-900 dark:text-secondary-100">
                                            {{ $editFilePreview['name'] }}</p>
                                        <p class="text-xs text-secondary-500 dark:text-secondary-400">
                                            {{ $editFilePreview['size'] }}</p>

                                        @if (isset($editFilePreview['key_count']) && $editFilePreview['is_valid_json'])
                                            <div
                                                class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-success-100 text-success-800 dark:bg-success-900/50 dark:text-success-300">
                                        ...
                                        @elseif(isset($editFilePreview['error']))
                                            <div
                                                class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-danger-100 text-danger-800 dark:bg-danger-900/50 dark:text-danger-300">
                                    ...
                                    <button type="button" wire:click="resetEditUploadState"
                            ...
                            @else
                                ...
                                <div class="space-y-3">
                                    <div class="flex items-center justify-center">
                                        <div
                                            class="w-12 h-12 bg-secondary-100 dark:bg-secondary-800 rounded-lg flex items-center justify-center">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-xs text-secondary-500 dark:text-secondary-400">
                                            {{ t('upload_maximum_file_size') }} <code
                                                class="bg-secondary-100 dark:bg-secondary-700 px-1 rounded">
                                                {{ $code ?? 'code' }}.json</code>
                                        </p>
                                        </p>
                                    </div>
                            ...
                            {{-- Hidden File Input --}}
                            <input type="file" wire:model="editUploadFile" accept=".json,application/json"
                                x-ref="editFileInput" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                        </div>
                    ...
                    @if ($editIsUploading)
                        <div class="space-y-2 mt-4">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span
                                    class="text-secondary-600 dark:text-secondary-400">{{ $editUploadProgress }}%</span>
                            </div>
                    ...
                    @if ($editUploadResults && !$editIsUploading)
                        <div
                            class="mt-4 p-4 {{ $editUploadResults['valid'] ? 'bg-success-50 dark:bg-success-900/20 border border-success-200 dark:border-success-800' : 'bg-danger-50 dark:bg-danger-900/20 border border-danger-200 dark:border-danger-800' }} rounded-lg">
                                ...
                                @if ($editUploadResults['valid'])
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="text-xs text-success-600 dark:text-success-400 space-y-1">
                                            <p>• {{ $editUploadResults['key_count'] }}
                                                {{ t('translation_keys_found') }}</p>
                                            <p>• {{ t('file_size') }}:
                                                {{ number_format($editUploadResults['file_size'] / 1024, 2) }} KB</p>
                                        </div>

                                        <div
                                            class="mt-3 p-2 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded">
                                            <p class="text-xs text-blue-700 dark:text-blue-300">
                                                <x-heroicon-m-information-circle class="w-3 h-3 inline mr-1" />
                                                Ready to upload to resource/lang/translations directory. Existing files
                                                will be backed
                                                up.
                                            </p>
                                        </div>
            ...
            <div
                class="py-4 flex justify-end items-center border-t border-neutral-200 dark:border-neutral-500/30 mt-6 px-6">
                <div class="flex items-center space-x-3">
                    ...
                    @if ($editUploadResults && $editUploadResults['valid'])
                        <x-button.primary type="button" wire:click="processEditUpload" :disabled="$editIsUploading"
                            ...
                            <span wire:loading.remove wire:target="processEditUpload"
                    ...
                    <x-button.loading-button type="submit" target="updateLanguage"
