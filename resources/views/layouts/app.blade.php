...
@php
    ...
    $themeSettings = get_batch_settings([
        ...
        'system.default_country_code',
        'theme.seo_meta_title',
        'theme.seo_meta_description',
        'theme.favicon',
    ]);
    $countryCode = $themeSettings['system.default_country_code'];
    $locale = Auth::check()
    ...
    // Create dateTimeSettings array with tenant-aware settings
    $dateTimeSettings = [
        'timezone' => $isTenant 
            ? ($systemSettings['timezone'] ?? config('app.timezone'))
            : ($themeSettings['system.timezone'] ?? config('app.timezone')),
        'dateFormat' => $isTenant 
            ? ($systemSettings['date_format'] ?? 'd-m-Y')
            : ($themeSettings['system.date_format'] ?? 'd-m-Y'),
        'timeFormat' => $isTenant 
            ? ($systemSettings['time_format'] ?? 'H:i')
            : ($themeSettings['system.time_format'] ?? 'H:i'),
        'is24Hour' => $isTenant 
            ? (($systemSettings['time_format'] ?? 'H:i') === 'H:i')
            : (($themeSettings['system.time_format'] ?? 'H:i') === 'H:i'),
    ];
...
<head>
    ...
    @php do_action('app_module_headers'); @endphp
    {{-- JS leaflet --}}
    <script>
        window.pusherConfig = {
        ...
        var timezone = window.dateTimeSettings.timezone;
...
<body class="h-full antialiased bg-gray-50 font-sans dark:bg-slate-800" x-data="{ theme: $persist('light') }" x-init="if (theme === 'dark' || (theme === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
    ...
    <script>
        ...
        window.tablePaginationSettings = @json(table_pagination_settings());
    </script>
    
    @vite(['resources/js/app.js', request()->routeIs('tenant.*') ? 'resources/js/tenant-app.js' : 'resources/js/admin-app.js'])

    @php do_action('app_module_footers'); @endphp
